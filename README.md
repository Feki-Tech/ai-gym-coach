...
- **The interactive coach on the iPhone** — `python coach_server.py` pairs the phone with the desktop coach over Wi‑Fi (same persona, RAG, guardrails, tools; standard library, selftested): talk by voice (hold the mic, on-device speech recognition) or text, streamed spoken answers with barge-in, proactive greetings/debriefs, and the coach drives the phone's workout — exercise switch, rep goal, rest, tempo, cues, load, guided programs. The phone HUD gained the ROM gauge, fault highlighting, rest overlay, goal ring, program strip, load/e1RM/PRs; finished sets sync into the desktop log; the camera is switchable (front/back/ultra-wide, external USB-C cameras on iOS 17+) and so is the microphone (AirPods, headsets, USB-C) — see [docs/IOS.md §8](docs/IOS.md)
