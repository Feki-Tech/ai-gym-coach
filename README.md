...
- **Coach LLMOps** — the coach is versioned, traced and tested like the classifier: a deterministic safety guardrail for red-flag symptoms (six languages), an opt-in local trace of latency + guardrail flags (`COACH_TRACE=…`, no text by default), `python coach_eval.py` running 31 behaviour scenarios (safety, language, number grounding, app actions, calendar, events) against your local model with a baseline gate, and `python coach_ops.py --doctor` — all local files, nothing uploaded — see [docs/LLMOPS.md](docs/LLMOPS.md)
...
- [x] Coach LLMOps — prompt versioning, local trace, deterministic guardrails, 31-scenario behaviour eval with baseline gate, `coach-eval` workflow — see [docs/LLMOPS.md](docs/LLMOPS.md)
