...
| Eval set | `data/coach_evals.jsonl` (31 scenarios, 7 categories) | declarative: safety, language, grounding, actions, calendar, events, style. Each row is context (history fixture, profile facts, live state, protocols on/off) + the athlete's message + `expect` checks. Add a case by adding a line. |
